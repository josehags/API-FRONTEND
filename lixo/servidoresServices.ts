import { APIServidores } from '../src/Services/Axios/baseService/index';

export async function getClients(url, startModal) {
  try {
    const response = await APIServidores.get(url);
    return response;
  } catch (error) {
...
export async function getClientData(id, startModal) {
  try {
    const response = await APIServidores.get(`clients/${id}`);
    return response;
  } catch (error) {
    if (error.response?.status === 500) {
      startModal('O tempo da sua sessão expirou, faça o login novamente');
...
export async function getFourClients(startModal) {
  try {
    const response = await APIServidores.get('/clients/newest-four');
    return response;
  } catch (error) {
    if (error.response?.status === 500) {
      startModal('O tempo da sua sessão expirou, faça o login novamente');
...
export async function postClient(
  ...
  try {
    const response = await APIServidores.post('clients/create', {
      name: inputName,
      email: inputEmail,
      cpf: inputCpf,
      phone: inputPhone,
...
export async function updateClient(
  ...
  try {
    const response = await APIServidores.put(`/clients/update/${id}`, {
      name: inputName,
      email: inputEmail,
      cpf: inputCpf,
      phone: inputPhone,
...
export const toggleStatus = async (id, startModal) => {
  try {
    await APIServidores.put(`/clients/toggleStatus/${id}`);
  } catch (error) {
    console.error(error);
...
export const getFeatures = async (url, startModal) => {
  try {
    const res = await APIServidores.get(url);
    return res;
  } catch (error) {
    if (error.response?.status === 500) {
      startModal('O tempo da sua sessão expirou, faça o login novamente');
...
export const getClientFeatures = async (featuresList, startModal) => {
  try {
    const res = await APIServidores.post('/featuresbyid', {
      featuresList,
    });
    return res;
  } catch (error) {
...
export const createFeature = async (
  ...
  try {
    const res = await APIServidores.post('feature/create', {
      name,
      description,
      color,
    });
...
export const updateFeature = async (
  ...
  try {
    const res = await APIServidores.put(`feature/update/${id}`, {
      name,
      description,
      color,
    });
...
export const deleteFeature = async (id, startModal) => {
  try {
    const res = await APIServidores.patch(`/feature/desactive/${id}`);
    return res;
  } catch (error) {
    if (error.response.status === 500) {
      startModal('O tempo da sua sessão expirou, faça o login novamente');
...
export async function createWorkspace(name, description, startModal) {
  try {
    const response = await APIServidores.post('lotacao/create', {
      name,
      description,
    });
...
export const updateWorkspace = async (
  ...
  try {
    const res = await APIServidores.put(`lotacao/update/${id}`, {
      name,
      description,
    });
    if (res.data.status) {
...
export const deleteWorkspace = async (id, startModal) => {
  try {
    const res = await APIServidores.delete(`/lotacao/delete/${id}`);
    return res;
  } catch (error) {
    if (error.response.status === 500) {
      startModal('O tempo da sua sessão expirou, faça o login novamente');
...
export const deactivateWorkspace = async (id, startModal) => {
  try {
    const res = await APIServidores.put(`/lotacao/deactivate/${id}`);
    return res;
  } catch (error) {
    if (error.response.status === 500) {
      startModal('O tempo da sua sessão expirou, faça o login novamente');
